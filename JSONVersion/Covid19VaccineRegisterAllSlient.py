def reg(vacCode, orgCode, Kakaocookie):
    typeDic = {"VEN00013": "화이자", "VEN00014": "모더나", "VEN00015": "아스트라제네카", "VEN00016": "얀센"}
    ...
    req = requests.post("https://vaccine.kakao.com/api/v1/reservation", json=data, headers=header, verify = False)
    req.request
    se = req.json()
    if se['code'] == "SUCCESS":
        ...
        print(f"접종하게 되는 백신은 {typeDic[vacCode]}입니다.")
        return True
        ...
        return False
print("COVID-19 잔여백신 자동 신청 프로그램 - mRNA Random Selection/Slient Version")
print("======= 주 의 사 항 =======")
...
if res.status_code == 401:
    ...
    print("카카오 서버로부터 과도한 요청으로 접속이 제한된 상태입니다.")
    exit()
...
print("Slient 버전입니다. 출력에 소요되는 시간을 절약하여 성공 확률을 늘립니다. 정상 작동 여부를 확인할 수 있도록 1,000회 시도시 마다 시도한 횟수를 출력합니다.")
count = 1
while True:
    if count % 1000 == 0:
        print(f"{count}회 실행 중입니다.")
    if reg("VEN00013", orgCode, Kakaocookie):
        break
    if reg("VEN00014", orgCode, Kakaocookie):
        break
    if reg("VEN00015", orgCode, Kakaocookie):
        break
    if reg("VEN00016", orgCode, Kakaocookie):
        break
    count += 1
